...
import {
  EditWhatsappReceiverSchema,
  type IEditWhatsappReceiver,
  type IWhatsappReceiverDetail,
} from '@/schemas/models';

import type { Route } from './+types';
import { EditWaDetail } from './contents/edit-wa-detail';

export function meta({}: Route.MetaArgs) {
  return generateMeta('WhatsApp Receiver Detail', 'Edit WhatsApp Receiver Detail');
}

export async function clientLoader({ params: { id } }: Route.ClientLoaderArgs) {
  try {
    const response = await api.get<IWhatsappReceiverDetail>(`/report/wa/${id}`);
...
export default function WaDetailPage({ loaderData }: Route.ComponentProps) {
  ...
  const methods = useRemixForm<IEditWhatsappReceiver>({
    mode: 'onBlur',
    defaultValues: {
      name: receiver?.name ?? '',
      wa_chat_id: receiver?.wa_chat_id ?? '',
      is_group: receiver?.is_group ?? false,
      is_activated: receiver?.is_activated ?? true,
    },
    submitHandlers: {
      onValid: async (data) => {
        try {
          await api.patch(`/report/wa/${receiver?.id}`, data);
          toast.success('WhatsApp Receiver updated successfully');
    ...
    resolver: valibotResolver(EditWhatsappReceiverSchema),
  });
  ...
  return (
    <>
      {receiver && (
        ...
        >
          <TitleSection
            title="WhatsApp Receiver Detail"
          ...
          <ConfirmDialog
            ...
            onConfirm={async () => {
              try {
                ...
                await api.patch(`/report/wa/${receiver.id}`, { is_activated: newStatus });
